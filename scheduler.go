...
func (s *vmScheduler) do(ctx context.Context, act *Action) (interface{}, error) {
	...
	defer func() {
		...
		metricCounter("glua_action_scheduler_second_total", time.Since(ts).Milliseconds(), nil)

		// metricCounter("glua_action_scheduler_second_total", time.Now().Sub(ts).Milliseconds(), nil)
	}()
